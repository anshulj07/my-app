...
const [bannerUri, setBannerUri] = useState<string | null>(null);
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      fetchAutocomplete({ key: GOOGLE_KEY, q, setLoading: setLoadingSug, setList: setSuggestions, setErr });
    }, 250);
  ...
  const handlePickSuggestion = async (s: Suggestion) => {
    if (!GOOGLE_KEY) return;
    Keyboard.dismiss();
    setErr(null); setLoadingSug(true); setLocLoading(true);
    try {
      const details = await fetchPlaceDetails(GOOGLE_KEY, s.id);
      if (!details?.latLng) { setErr("Couldn't get coordinates for that place."); return; }
      const addr = details.formattedAddress || [s.main, s.secondary].filter(Boolean).join(", ");
      setCoord(details.latLng); setSelectedAddress(addr); setQuery(addr); setSuggestions([]);
      const loc = buildLocationFromAddressComponents({
        lat: details.latLng.lat, lng: details.latLng.lng,
        formattedAddress: addr, placeId: s.id,
        components: details.addressComponents ?? [], source: "places_autocomplete",
      });
      if (!loc?.countryCode || !loc?.city) { setLocationPayload(null); setErr("Couldn't extract city/country."); return; }
      setLocationPayload({ ...loc, lat: details.latLng.lat, lng: details.latLng.lng, formattedAddress: addr, placeId: s.id, source: "places_autocomplete" });
    } catch { setErr("Something went wrong while selecting that place."); }
    finally { setLoadingSug(false); setLocLoading(false); }
  };
  ...
  const onMapMessage = async (e: any) => {
    try {
      ...
      if (msg?.type === "picked" && typeof msg.lat === "number" && typeof msg.lng === "number") {
        ...
        setCoord(picked); setSuggestions([]); setErr(null);
        if (!GOOGLE_KEY) return;
        setLocLoading(true); setSelectedAddress("Dropped pin");
        const geo = await reverseGeocode(GOOGLE_KEY, picked.lat, picked.lng);
        if (!geo?.formattedAddress || !geo?.components?.length) { setSelectedAddress("Dropped pin (no address found)"); return; }
        setSelectedAddress(geo.formattedAddress);
        const loc = buildLocationFromAddressComponents({
          lat: picked.lat, lng: picked.lng, formattedAddress: geo.formattedAddress,
          placeId: geo.placeId, components: geo.components, source: "reverse_geocode",
        });
        setLocationPayload({
          ...loc, lat: picked.lat, lng: picked.lng, formattedAddress: geo.formattedAddress,
          placeId: geo.placeId ?? "", source: "reverse_geocode",
          countryCode: loc?.countryCode ?? "", countryName: loc?.countryName ?? "",
          admin1: loc?.admin1 ?? "", admin1Code: loc?.admin1Code ?? "",
          city: loc?.city ?? "", cityKey: loc?.cityKey ?? "",
          postalCode: loc?.postalCode ?? "", neighborhood: loc?.neighborhood ?? "",
        });
      }
    } catch { }
  ...
  async function createListing() {
    ...
    const payload = {
      ...
      location: {
        ...
        countryCode: locationPayload.countryCode, countryName: locationPayload.countryName || "",
        admin1: locationPayload.admin1 || "", admin1Code: locationPayload.admin1Code || "",
        city: locationPayload.city, cityKey: locationPayload.cityKey || "",
  ...
  const handleCreate = async () => {
    setErr(null);
    if (!GOOGLE_KEY) { setErr("Google Maps key missing (extra.googleMapsKey)."); return; }
...
setBannerUri={setBannerUri}
        setKind={(k) => {
          ...
          else { setLimitEnabled(false); setCapacityText(""); }
          setErr(null);
        }}
        priceText={priceText}
