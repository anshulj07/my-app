...
export function buildMapHtml(args: {
  ...
  <body>
    ...
    <script>
      ...
      // ✅ Listen for goToLocation message from React Native (current location button)
      window.addEventListener("message", function(e) {
        try {
          var msg = JSON.parse(e.data);
          if (msg && msg.type === "goToLocation" && map) {
            var lat = Number(msg.lat);
            var lng = Number(msg.lng);
            ...
            // ✅ Remove old blue dot if exists
            if (window._currentLocOverlay) {
              window._currentLocOverlay.setMap(null);
              window._currentLocOverlay = null;
            }

            // Inject pulse animation once
            if (!document.getElementById("locPulseStyle")) {
              var s = document.createElement("style");
              s.id = "locPulseStyle";
              s.textContent = [
                "@keyframes locPulse{",
                "0%,100%{box-shadow:0 0 0 4px rgba(10,132,255,0.15)}",
                "50%{box-shadow:0 0 0 10px rgba(10,132,255,0.04)}",
                "}"
              ].join("");
              document.head.appendChild(s);
            }

            // ✅ Blue dot overlay (same pattern as emoji pins)
            var locOverlay = new google.maps.OverlayView();
            locOverlay._pos = new google.maps.LatLng(lat, lng);

            locOverlay.onAdd = function() {
            ...
            window._currentLocOverlay = locOverlay;
          }
        } catch (_) {}
      });
