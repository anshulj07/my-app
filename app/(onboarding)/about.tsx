...
export default function AboutScreen() {
  ...
  const initial = (user?.publicMetadata as any)?.about;
  const [about, setAbout] = useState(typeof initial === "string" ? initial : "");
  ...
  const onNext = async () => {
    ...
    try {
      ...
      setErr(e?.errors?.[0]?.longMessage || e?.message || "Failed to save about.");
    } finally {
      setSaving(false);
    }
  };
